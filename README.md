...
animal is impractical. Yet many health problems, such as lameness, metabolic stress,
illness, and reduced feeding, first show up as subtle, gradual changes in an individual
cow's behaviour and physiology. These early signs are easily missed, and by the time a
cow is visibly unwell, welfare and milk yield have often already suffered.
...
| Body condition (BCS) | Bounding-box-area proxy, standing frames only | Relative ranking (2.0 to 4.0 band) | **No** |
...
| Dashboard | Streamlit + Plotly, reads precomputed outputs only | Herd Overview + Per-Cow Detail pages | n/a |
...
lifetime cow records spanning 2014 to 2024, with zero medically relevant events near the
...
walk-throughs of individual cows through the entire pipeline are in `evaluation/`:
[**consolidated_metrics.md**](evaluation/consolidated_metrics.md) (all 5 stages' results
in one table) and [**case_studies.md**](evaluation/case_studies.md) (4 cows traced
end-to-end through the pipeline), generated by `src/10_make_confusion_matrix.py`.

**Behaviour classifier, confusion matrix (test set, normalised by row):**

![Behaviour confusion matrix](evaluation/confusion_matrix_normalised.png)
...
Vision-only cows (C11 to C16, no wearable sensors) are handled explicitly throughout;
sensor fields are shown as clearly marked "N/A" rather than blank or `NaN`.

**Herd Overview:**

![Herd Overview](docs/dashboard_herd_overview.jpeg)

**All Cows table, sortable, colour-coded, CSV export:**

![All Cows table](docs/dashboard_all_cows_table.jpeg)

**Per-Cow Detail, reference photo, behaviour donut, sensor metrics:**

![Per-Cow Detail](docs/dashboard_per_cow_detail.jpeg)

**Health screening, rule breakdown (which of the 7 flags fired):**
...
bosight/
├── src/
│   ├── 00_data_setup.ipynb          # Drive mount, MmCows extraction, folder verification
...
│   ├── consolidated_metrics.md      # all 5 stages' results in one table
│   ├── confusion_matrix.png
│   ├── confusion_matrix_normalised.png
│   └── confusion_matrix.csv
├── docs/
│   ├── dashboard_herd_overview.jpeg
│   ├── dashboard_all_cows_table.jpeg
│   ├── dashboard_per_cow_detail.jpeg
│   └── dashboard_rule_breakdown.jpeg
├── models/
...
- **Vision-only cows (C11 to C16)** are screened on behaviour and BCS alone, with no
...
Information Processing Systems (NeurIPS) 37, 59451 to 59467.
